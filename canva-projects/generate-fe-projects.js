...
function descriptionFromText(text) {
  ...
  const contentLines = lines.filter((l) => !/^PROJECTS\s*$/i.test(l) && !/^[\d\s\|=—\-]+$/.test(l) && l.length > 10);
...
function main() {
  ...
  if (!fs.existsSync(PUBLIC_PROJECTS_IMAGES)) fs.mkdirSync(PUBLIC_PROJECTS_IMAGES, { recursive: true });
